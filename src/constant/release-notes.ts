export const ReleaseNotesContent = [
	{
		version: '8.4.0',
		accent: '#F58B00',
		imageSource: "v8_4_0/header.png",
		imageAlt: "Une bicyclette, un bus urbain et le symbole d'un panneau « Stop », symbolisant les transports urbains et la circulation.",
		title: 'Quoi de nouveau\ndans Papillon v8.4 ?',
		lastUpdate: '29/03/2026',
		cards: [
			{
				color: '#F58B00',
				imageSource: "v8_4_0/transport.png",
				imageAlt: '',
				title: 'Tes transports dans ton emploi du temps',
				contributors: ['remy', 'vince'],
				description: 'Tes trajets s’invitent dans ton emploi du temps. Tu sais désormais exactement quand partir pour être à l’heure en cours. Plus aucune excuse pour être en retard…'
			},
			{
				color: '#26B290',
				imageSource: "v8_4_0/account_sharing.png",
				imageAlt: '',
				title: 'Tout tes comptes, au même endroits',
				contributors: ['vince', 'raphael'],
				description: 'Tu as plusieurs rôles ou services à suivre ? Que tu sois prof et parent d\'élève ou même étudiant avec plusieurs comptes, pas de problème. Papillon supporte désormais plusieurs comptes au sein du même endroit. Tu peux passer de l’un à l’autre en un clin d’œil et garder toutes tes infos à portée de main. '
			},
			{
				color: '#B300CA',
				imageSource: "v8_4_0/languages.png",
				imageAlt: '',
				title: '¡Muchas novedades lingüísticas en el mismo sitio!',
				contributors: ['vince'],
				description: 'Parles-tu 2, 3 ou 4 langues ? Peu importe. Papillon supporte désormais 32 nouvelles langues, de quoi tout le monde trouve son bonheur.'
			},
			{
				color: '#2DB9D8',
				imageSource: "v8_4_0/new_onboarding.png",
				imageAlt: '',
				title: 'Simple comme bonjour !',
				contributors: ['vince', 'tom'],
				description: 'L\'écran de connexion s\'est refait une beauté  ! Plus clair, plus simple et beaucoup plus agréable à utiliser, tu peux maintenant te connecter en un clin d’œil. Même ton chat pourrait y arriver !'
			}
		]
	}
]
