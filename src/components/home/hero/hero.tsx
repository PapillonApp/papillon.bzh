...
export default function Hero() {
	...
	useEffect(() => {
		var delay = 0;
		const titleAnimtedPart = document.getElementsByClassName("main-title-animated-part");
		// Add a span around each letter
		for (let i = 0; i < titleAnimtedPart.length; i++) {
			const text = titleAnimtedPart[i].textContent;
			if (!text) continue;
			const split = text.split("");
			titleAnimtedPart[i].innerHTML = split
				.map((char) => {
					delay += 0.01;
					if (char === " ") {
						return "<span class=\"char\" style=\"display: inline-block; width: 0.3em;\">&nbsp;</span>";
					}
					return `<span class="char" style="display: inline-block; animation-delay: ${delay}s">${char}</span>`;
				})
				.join("");
		}
	}, []);
		...
		<>
			<div className="hero">
				<div className="hero-content width">
					...
					>
						<span className={"main-title-animated-part"}>L’application ultime pour les{" "}</span>
						<HeroSpan
							text="cours"
							color="#B84C00"
							icon={<Calendar/>}
							index={1}
						/>
						<span className={"main-title-animated-part"}>,</span>
						<span className={"main-title-animated-part"}>{" "}tes{" "}</span>
						<HeroSpan
							text="tâches"
							color="#B1399F"
							icon={<Check/>}
							index={2}
						/>
						<span className={"main-title-animated-part"}>{" "}et tes{" "}</span>
						<HeroSpan
							text="résultats"
							color="#006F85"
							icon={<Pie/>}
					...
					<motion.p
						...
						transition={transition(0.1)}
					>
						Papillon est une alternative fiable, <span>libre et open-source</span> aux applications de vie
						scolaire traditionnelles, créée par des élèves et des étudiants tout comme toi. Utilise-le dès
						aujourd’hui pour tous tes services de vie scolaire, et simplifie ton année !
					</motion.p>
					...
					>
						En utilisant l’application Papillon, vous confirmez avoir lu et accepté
						les <a href="https://docs.papillon.bzh/terms">Termes et conditions</a>.<br/>
